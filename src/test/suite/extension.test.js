...
describe("Extension Tests", function () {
	...
	it("should activate extension successfully", async () => {
		// Get the extension
		const extension = vscode.extensions.getExtension("saoudrizwan.rich-cline")
	...
	it("should open sidebar view", async () => {
		...
		await vscode.commands.executeCommand("rich-cline.plusButtonClicked")
	...
	it("should handle basic commands", async () => {
		...
		await vscode.commands.executeCommand("rich-cline.historyButtonClicked")
		// Success if no error thrown
	})

	it("should handle advanced settings configuration", async () => {
		// Test browser session setting
		await vscode.workspace.getConfiguration().update("rich-cline.disableBrowserTool", true, true)
		const updatedConfig = vscode.workspace.getConfiguration("rich-cline")
		expect(updatedConfig.get("disableBrowserTool")).to.be.true

		// Reset settings
		await vscode.workspace.getConfiguration().update("rich-cline.disableBrowserTool", undefined, true)
	})
})
