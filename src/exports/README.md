...
The Rich Cline extension exposes an API that can be used by other extensions. To use this API in your extension:
...
3. Get access to the API with the following code:
    ...
    ```ts
    const clineExtension = vscode.extensions.getExtension<ClineAPI>("saoudrizwan.rich-cline")

    if (!clineExtension?.isActive) {
    	throw new Error("Rich Cline extension is not activated")
    }
    ...
    if (cline) {
    	...
    	await cline.startNewTask("Hello, Rich Cline! Let's make a new project...")
    	...
    	console.error("Rich Cline API is not available")
    }
    ```

    **Note:** To ensure that the `saoudrizwan.rich-cline` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

    ```json
    "extensionDependencies": [
        "saoudrizwan.rich-cline"
    ]
    ```
